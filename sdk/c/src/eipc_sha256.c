/*
 * EIPC SHA-256 Implementation (FIPS 180-4)
 * Standalone, zero-dependency SHA-256 hash function.
 */
...
static void sha256_final(sha256_ctx *ctx, uint8_t hash[32]) {
    size_t i;
    uint64_t total_bits;
    size_t orig_len = ctx->block_len;

    total_bits = ctx->total_bits + (uint64_t)orig_len * 8;
...
void eipc_sha256(const uint8_t *data, size_t len, uint8_t hash[32]) {
    sha256_ctx ctx;
    sha256_init(&ctx);
