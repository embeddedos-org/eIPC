...
eipc_status_t eipc_client_send_chat(eipc_client_t *c,
                                    const eipc_chat_request_t *req) {
    eipc_header_t hdr;
    char payload_json[EIPC_MAX_PAYLOAD];
    eipc_status_t rc;

    if (!c || !c->connected || !req) return EIPC_ERR_INVALID;
...
eipc_status_t eipc_client_send_complete(eipc_client_t *c,
                                        const char *prompt,
    ...
    if (!c || !c->connected || !prompt) return EIPC_ERR_INVALID;
    ...
    if (n < 0 || (size_t)n >= sizeof(payload_json)) return EIPC_ERR_FRAME_TOO_LARGE;

    return build_and_send(c, EIPC_MSG_COMPLETE, &hdr,
                          (const uint8_t *)payload_json, (size_t)n);
}
