"""eIPC — production promo video with synced narration."""
from manim import *
import json
import os
...
class ProductPromo(Scene):
    def construct(self):
        ...
        # ═══ INTRO ═══
        title = Text("eIPC", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("Embedded Inter-Process Communication", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        # Tech badges
        techs = "C, POSIX, FreeRTOS".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # ═══ FEATURES ═══
        features = [
            ("01", "Shared Memory Channels", "Lock-free ring buffers deliver microsecond latency between processes", DUR["f1"]),
            ("02", "Zero-Copy Message Passing", "Memory-mapped transfers eliminate serialization and copy overhead", DUR["f2"]),
            ("03", "Cross-Platform Portability", "Runs on Linux, FreeRTOS, Zephyr, and bare metal with the same API", DUR["f3"]),
        ]
        ...
        components = ["Channel", "Ring Buffer", "Serializer", "Transport", "Discovery"]
        boxes = VGroup()
