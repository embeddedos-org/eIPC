...
type RingBuffer struct {
	...
	head     atomic.Uint64
	tail     atomic.Uint64
	hmacKey  []byte // When set, frames are HMAC-signed on write and verified on read
}

// SetHMACKey enables HMAC-SHA256 signing/verification for SHM frames.
func (rb *RingBuffer) SetHMACKey(key []byte) {
	rb.hmacKey = make([]byte, len(key))
	copy(rb.hmacKey, key)
}
